...
router.post('/generate-plus', async (req, res) => {
  console.log('[VideoSegments Generate Plus] Request received:', {
  ...
  try {
    ...
    if (!script || script.trim().length < 50) {
      console.log('[VideoSegments Generate Plus] Validation failed: Script too short');
    ...
    try {
      ...
      console.error('[VideoSegments Generate Plus] Failed to persist run files:', persistErr);
    }

    console.log('[VideoSegments Generate Plus] Success:', {
    ...
    console.error('[VideoSegments Generate Plus] Error:', {
...
router.post('/download-plus', async (req, res) => {
  try {
    ...
    console.error('[VideoSegments] Download Plus error:', error);
